...
func (rep *Repository) SoftDeleteClaimDefinition(w http.ResponseWriter, r *http.Request) {
	...
	for _, eid := range p.List {
		...
		err = rep.App.Models.ClaimDefinition.Delete(id)
		if err != nil {
			rep.errorJSON(w, err)
			return
		}
	...
	answer := jsonResponse{
		Error:     false,
		Message:   "claim definition successfully deleted",
		Data:      p.List,
